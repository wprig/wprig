...
describe( 'Block Theme Validators', () => {
	...
	describe( 'Theme JSON', () => {
		...
		it( 'warns when preset slugs collide with core defaults', () => {
			const findings = themeJson.run(
				path.join( fixturesPath, 'collision-theme' ),
				'collision-theme'
			);
			const collisions = findings.filter( ( f ) =>
				f.id.startsWith( 'theme-json.core-preset-collision.' )
			);
			expect( collisions ).toHaveLength( 2 );
			expect(
				collisions.every( ( f ) => f.severity === 'WARNING' )
			).toBe( true );
			expect(
				collisions.some(
					( f ) =>
						f.id === 'theme-json.core-preset-collision.color.black'
				)
			).toBe( true );
			expect(
				collisions.some(
					( f ) =>
						f.id ===
						'theme-json.core-preset-collision.fontSize.small'
				)
			).toBe( true );
		} );
