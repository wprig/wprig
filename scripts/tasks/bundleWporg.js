...
export default async function runBundleWporg(opts = {}) {
	...
	await runBundle({ phpcs: false, lint: false });

	const prodThemePath = path.join(
		process.cwd(),
		'..',
		config.theme.slug
	);
	...
	if (!opts.skipAudit) {
		...
		try {
			...
			const auditScriptPath = path.join(process.cwd(), 'scripts', 'theme-review', 'run.js');
			...
			console.error('The ZIP file was still generated for inspection, but it is NOT submission-ready.');
