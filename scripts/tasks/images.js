...
function getDestRoot() {
	return paths.images.dest.endsWith('/') ? paths.images.dest.slice(0, -1) : paths.images.dest;
}
...
export async function images() {
	...
	const files = await fg(patterns, { caseSensitiveMatch: false, dot: false, onlyFiles: true });

	for ( const file of files ) {
		const dest = destPathFor(file);
		if ( !(await isNewer(file, dest)) ) continue;
...
export async function convertToWebP() {
	...
	for ( const file of files ) {
		...
		const destFile = path.join( getDestRoot(), rel ).replace(/\.[^.]+$/i, '.webp');
		if ( !(await isNewer(file, destFile)) ) continue;
		try {
			await fse.ensureDir( path.dirname(destFile) );
			...
			console.error(`Failed to convert to WebP: ${ path.basename(file) }`, err);
		}
	}
}
