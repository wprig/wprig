...
function initEachNavToggleSmall( nav ) {
	...
	// Return early if MENUTOGGLE is missing or it is the primary menu toggle.
	if (
		! menuTOGGLE ||
		menuTOGGLE === document.querySelector( "#site-navigation .menu-toggle" )
	) {
		return;
	}
...
/**
 * Adds headings and collapse buttons for sliding menu.
 */
function setSubmenuTogglesAndHeadings() {
	...
	parentMenuItems.forEach( ( menuItem ) => {
		...
		expandBtn.setAttribute(
			"aria-label",
			wpRigScreenReaderText.expandChild
		);
		...
		backBtn.classList.add( "collapse-menu" );
...
/**
 * Expands or collapses Primary nav menu depending on screen width.
 */
...
/**
 * Expands and collapses primary menu depending on input.
 *
 * @param {object} primaryToggle Navigation primary toggle element
 * @param {bool} forceCollapsed collapse the menu
 */
function toggleNavigationPrimaryMenu( primaryToggle, forceCollapsed ) {
	if ( forceCollapsed ) {
		// hide it.
		primaryToggle.setAttribute( "aria-expanded", "false" );
		primaryToggle.setAttribute(
			"aria-label",
			wpRigScreenReaderText.expandPrimary
		);
		document
		...
		primaryToggle.setAttribute(
			"aria-label",
			wpRigScreenReaderText.collapsePrimary
		);
		document
			.querySelector( "#site-navigation" )
...
/**
 * Expands and collapses submenu depending on input.
 *
 * @param {object} submenu Navigation ul element
 * @param {bool} forceCollapsed collapse the menu
 */
function toggleNavigationSubmenu( submenu, forceCollapsed = false ) {
	const controlToggles = document.querySelectorAll(
		`#primary-menu button[aria-controls="${ submenu.id }"]`
	);

	if ( forceCollapsed || submenu.classList.contains( "toggled-on" ) ) {
		...
		controlToggles.forEach( ( toggle ) => {
			...
			toggle.setAttribute(
				"aria-label",
				wpRigScreenReaderText.collapseChild
			);
		} );
	}
}
