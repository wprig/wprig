...
class Component implements Component_Interface {
	...
	public static function is_active(): bool {
		...
		if ( isset( $_SERVER['HTTP_HOST'] ) ) {
			...
			if ( $host === 'localhost' || strpos( $host, '127.0.0.1' ) !== false || strpos( $host, '.test' ) !== false || strpos( $host, '.local' ) !== false || strpos( $host, '.dev' ) !== false ) {
				return true;
			}
		}
	...
	public function initialize() {
		...
		add_action( 'get_template_part', [ $this, 'inject_template_start_comment' ], 10, 2 );

		// Register HTML boundary comments for Gutenberg blocks
		add_filter( 'render_block', [ $this, 'inject_block_boundary_comments' ], 10, 2 );

		// Register the root template tracker
		add_filter( 'template_include', [ $this, 'track_root_template' ] );
	...
	public function track_root_template( string $template ): string {
		...
		add_action( 'wp_head', function() use ( $relative_template ) {
