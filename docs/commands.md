...
- `rig:tokens`: Regenerate `theme.json` + CSS variables + `@custom-media` + Tailwind from `config/tokens.json`. When a baked overlay exists (`config/user-styles.json`, written by `rig:bake`), the generator merges it last — tokens stay canonical, baked Global Styles/fonts survive every regeneration. `build:css` / `dev:css` / `lint:css` run this automatically.
- `rig:tokens:setup`: One-time token migration — upgrades a v1 flat `config/tokens.json` to the v2 layered schema and rewrites legacy custom-property names across CSS/PHP/JS (collapse-aware). Dry-run by default; pass `--apply` to write (timestamped backups land in `.rig-backup/`). Idempotent.
- `rig:tokens:import -- --from <file> [--mode merge|replace] [--apply]`: Import a W3C DTCG / Figma (Tokens Studio) JSON document into `config/tokens.json`. Reports unmapped tokens; dry-run by default.
- `rig:tokens:export [--out <file>]`: Export `config/tokens.json` to a DTCG document (default `design/tokens.dtcg.json`) for round-tripping back into Figma. See `docs/DESIGN.md`.
- `rig:bake`: Bake Site Editor database changes (unsynced patterns, custom templates/parts, Font Library fonts, Global Styles) into the theme's version-controlled files. Node port of [`bacoords/wp-theme-control`](https://github.com/bacoords/wp-theme-control) at `bin/wp-theme-control/`; paradigm-gated to `universal`/`block-based`. Runs fonts → templates → styles → patterns, review-first (no DB cleanup). Template export prefers `wp block template export` (WP-CLI 3.0) and auto-falls back to the template post content on WP-CLI 2.x.
