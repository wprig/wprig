...
config/tokens.json  ──(npm run rig:tokens)──▶  _tokens.generated.css   (gitignored, generated)
        ...
        └──▶ tailwind.config.js  (OPT-IN: config/tailwind.tokens.js + tailwind.custom.js)
```
...
| `config/tailwind.custom.js` | Hand-authored Tailwind extensions (created on opt-in) | yes |
| `config/tailwind.tokens.js` | Generated Tailwind values (opt-in, gitignored) | **never** |
| `tailwind.config.js` | Generated shell importing the two above (opt-in, gitignored) | **never** |
...
**WP Rig does not ship Tailwind** — it is not a dependency and nothing
Tailwind-related is generated by default. To opt in: install `tailwindcss`
yourself and set `theme.designTokens.emit.tailwind: true` in `config/config.json`,
then run `npm run rig:tokens`. That generates `config/tailwind.tokens.js` and a
`tailwind.config.js` shell, and creates `config/tailwind.custom.js` (a stub, if
absent) for your own extensions (per-key merge, custom wins).
