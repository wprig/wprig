...
async function createRigComponent() {
  try {
    ...
    if (await fileExists(componentFilePath)) {
      console.error(`Error: Component.php already exists at ${componentFilePath}`);
      process.exit(1);
    }
...
async function createTestFile(componentInfo) {
  const { pascalName, kebabSlug } = componentInfo;
  const testDir = path.join(themeRoot, 'tests', 'phpunit', 'unit', 'inc', pascalName);
...
async function wireComponent(componentInfo) {
  try {
    ...
    if (await fileExists(functionsPath)) {
      ...
      const registerComponentsRegex = /(register_components\s*\()\s*array\(([\s\S]*?)\)([\s\S]*?\));/;
      ...
      if (functionsMatch) {
        ...
        if (lastComponentPos !== -1) {
          ...
          if (endOfLinePos !== -1) {
            ...
            const newComponentLine = `\n\t\tnew ${componentInfo.pascalName}\\Component(),`;
            const newContent = functionsContent.slice(0, functionsMatch.index + functionsMatch[1].length + endOfLinePos + 1) +
                             newComponentLine +
                             functionsContent.slice(functionsMatch.index + functionsMatch[1].length + endOfLinePos + 1);
    ...
    console.warn(`to the components array in inc/Theme.php or inc/functions.php`);

  } catch (error) {
    console.warn(`Warning: Failed to auto-wire component: ${error.message}`);
    console.warn(`Please add the component manually to Theme.php or functions.php`);
  }
}
