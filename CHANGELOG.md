...
## 3.5.0
- **Design token system v2 (SPEC-017, `all`; wp-preset binding `block-based`):** `config/tokens.json` is now layered (primitives → semantic light/dark → component) with a JSON Schema, and generates a whole-file `assets/css/src/_tokens.generated.css` (regex marker surgery removed) alongside the hand-authored `_tokens.custom.css`; dark mode is generated from `semantic.*.dark` (fixing the previous gap where token colors never flipped). Config-driven `theme.designTokens.colorBinding` (`independent` vs `wp-preset`, paradigm-aware via `config/paradigms.json`; classic never emits `--wp--preset--` refs) + a WCAG contrast gate (warn). New `rig:tokens:setup` (v1→v2 upgrade + collapse-aware legacy-name codemod with backups), `rig:tokens:import`/`rig:tokens:export` (W3C DTCG / Figma round-trip), `config/theme.custom.json` + `config/tailwind.custom.js` source splits, stylelint `wprig/no-undefined-custom-properties` + `wprig/no-hardcoded-colors`, and `docs/DESIGN.md`. Generated artifacts (`_tokens.generated.css`, `config/tailwind.tokens.js`, `tailwind.config.js`, `artifacts/token-inventory.json`, `design/tokens.dtcg.json`) are gitignored because their content is config/paradigm-dependent; `build:css` / `dev:css` / `lint:css` regenerate tokens automatically. The framework source itself was migrated to canonical token names (legacy aliases default **off**; opt in with `theme.designTokens.legacyAliases` for un-migrated themes). **Known:** the default palette has 6 WCAG AA contrast warnings (link-hover, accent, quote-citation dark, state.success/warning/info) — the gate runs at `warn`. **Tailwind remains fully optional:** no dependency is added, and Tailwind config files are generated only when `theme.designTokens.emit.tailwind` is enabled. Props @robruiz
- **Navigation default state after `rig-init` (universal/block-based):** the starter template no longer ships a bare `<!-- wp:navigation /-->` — core renders markup inner blocks first and only an EMPTY nav triggers the lazy fallback path (most-recent `wp_navigation` lookup + auto-created "Navigation" page-list fallback post, rendered unstyled). The starter now ships inline `<!-- wp:home-link /-->` + `<!-- wp:page-list {"showSubmenuIcon":false} /-->` and the hardcoded `"ref":4` DB residue is gone — no database IDs in shipped templates, ever. `_blocks-based.css` adds a page-list-inside-navigation baseline (list reset, gap/color inheritance, hover/focus-visible/current-page affordances, nested-`<ul>` indent) so the default state renders styled like nav links. New opt-in `npm run seed:navigation` (`node/seedNavigation.js` + pure `scripts/lib/navigation-seed.js`): seeds a `wprig-primary-nav` post (migrating legacy `e2e-primary-nav`), prunes ONLY the exact untouched core fallback (fingerprint + never-edited + default title — never by title alone), 10s-bounded WP-CLI probe, graceful skip (CI / no `wp` / `WPRIG_SKIP_NAV_SEED=1`), never fails init. Jest guards assert starter templates never contain bare nav markup or numeric refs. Props @robruiz
- **True-classic guarantee on `rig-init` (all/classic):** the paradigm switch path now enforces what the choice promises. A stale `config/config.local.json` override (`theme.themeType` / `theme.enableBlocks`) no longer silently wins the merge chain — init strips those two keys (all other keys preserved) on every run so the interactive choice is authoritative at both build and runtime. Selecting **classic** additionally moves leftover `templates/` + `parts/` FSE artifacts into a timestamped `.rig-backup/<timestamp>/` folder (never deleted) — a stray `templates/index.html` made WordPress core report `wp_is_block_theme() === true` and surface the Site Editor regardless of WP Rig config. New shared module `scripts/lib/paradigm-switch.js` + jest coverage. `theme.json` is intentionally kept (hybrid-legitimate; does not trigger the Site Editor). Props @robruiz
- **Site Editor bake & sync (`rig:bake`, Track G, block-based):** Node port of [`bacoords/wp-theme-control`](https://github.com/bacoords/wp-theme-control) (upstream SHA `0820d965`) shipped at `bin/wp-theme-control/` — bakes unsynced patterns, custom templates/parts (runtime-URL content patternizes to `patterns/hidden-*.php`), Font Library fonts (magic-byte validated, `file:./` src rewrites), and Global Styles into theme files with manifest + sha256-verified cleanup. Paradigm-gated to `universal`/`block-based`; classic gets a graceful refusal. New commands `rig:bake` / `rig:bake:plan` / `rig:bake:clean` with full arg passthrough, plus the post-bake layer: idempotent `Baked: yes` headers and the overlay empty-state rule. The templates scope prefers `wp block template export` (WP-CLI 3.0) and auto-falls back to the template post content on WP-CLI 2.x. Requires Node 18+ and WP-CLI 2.12+. Props @robruiz
- **User-styles overlay (SPEC-016):** baked Global Styles and font activations land in git-tracked `config/user-styles.json`, not `theme.json`. `buildThemeJson( tokens, existing, overlay )` merges the overlay last (deep-merge objects, arrays replace wholesale, fonts collapse with custom definitions winning on duplicate slugs); SSOT keys (`version`, `$schema`, `settings.viewport`, `settings.blockVisibility`, `isGlobalStylesUserThemeJSON`) never merge. tokens.js remains the sole `theme.json` writer and `rig:tokens` regeneration is deterministic after a bake. PHP runtime is overlay-blind (PHPUnit parity guard). Props @robruiz
- **Lint tolerances for baked files (block-based):** patterns marked `Baked: yes` keep Title/Slug/Categories + markup validity checks but scaffold-placeholder content downgrades to a warning; `validate-block-markup` neutralizes the three known runtime-URL PHP expressions in block-comment attributes (`get_stylesheet_directory_uri()`, `wp_get_upload_dir()['baseurl']`, `home_url()`) — any other PHP still fails. `patterns/` is now git-tracked so baked files are reviewable. Props @robruiz
- **Fonts boundary + preload guard:** strict preload scoping in `inc/Fonts` — `get_font_files_to_preload()` only preloads families shipped by the component's own download pipeline (marked by `google-fonts.css`), never Font Library files baked into the same directory by `rig:bake` (those are served via theme.json). Boundary documented in `docs/advanced-features.md` ("Theme Fonts vs Baked Fonts") + the bake-sync skill: one home per font family (component CSS vs baked overlay). Props @robruiz
...
- **Docs & skills synced to v3.5:** refreshed `docs/workflow.md` (Lightning CSS, sharp/AVIF image pipeline, paradigm config), `docs/commands.md` (complete current script reference), `docs/architecture.md` (manifest-driven component discovery + paradigm system), `docs/advanced-features.md` (Lightning CSS, PHP-only blocks, WP-CLI bridge), `docs/wordpress-7-1-editor-iframe.md` (apiVersion:3 items marked resolved), `docs/testing.md` (3-browser matrix + WebKit notes), plus the `npm-scripts`, `architecture`, `code-quality-standards`, `component-registry`, `child-theme-development`, `web-designer`, `pull-request-authoring`, `phpunit-testing`, and `theme-bundling` skills and `.ai/PROJECT_RULES.md`. Props @robruiz
