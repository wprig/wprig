...
## 3.5.0
- **Site Editor bake & sync (`rig:bake`, Track G, block-based):** Node port of [`bacoords/wp-theme-control`](https://github.com/bacoords/wp-theme-control) (upstream SHA `0820d965`) shipped at `bin/wp-theme-control/` — bakes unsynced patterns, custom templates/parts (runtime-URL content patternizes to `patterns/hidden-*.php`), Font Library fonts (magic-byte validated, `file:./` src rewrites), and Global Styles into theme files with manifest + sha256-verified cleanup. Paradigm-gated to `universal`/`block-based`; classic gets a graceful refusal. New commands `rig:bake` / `rig:bake:plan` / `rig:bake:clean` with full arg passthrough, plus the post-bake layer: idempotent `Baked: yes` headers and the overlay empty-state rule. Requires Node 18+ and WP-CLI 3.0+ (`wp block template export`). Props @robruiz
