...
| Theme tokens → theme.json / CSS (+ opt-in Tailwind) | `config/tokens.json` (v2 layered), `config/theme.custom.json`, `scripts/tasks/tokens.js` |
...
| `config/tokens.json` | Design tokens (v2: primitives → semantic → component) — single source of truth for colors/fonts/spacing/breakpoints (propagated to `theme.json` + CSS; Tailwind only when opted in). See `docs/DESIGN.md` |
| `config/theme.custom.json` | Hand-authored `theme.json` fragments (merged over tokens, under the user layer) |
| `config/tailwind.custom.js` | Hand-authored Tailwind extensions (opt-in only) |
