...
### 4.1 Required Core Hooks & Template Files
* **Mandatory Enqueueing:**
	...
	* No hardcoded `<script>` or `<link rel="stylesheet">` tags in `header.php` or template files.
...
* **Title Tag Support:**
	* Theme must declare support for core title tags in `functions.php`: `add_theme_support('title-tag')`.
	* No hardcoded `<title>` tag in `header.php`.

### 4.2 Core Features & Block Editor (Gutenberg) Compatibility
* **Block Theme / Hybrid / Classic Parity:**
	* Must support block editor alignments (`alignwide`, `alignfull`).
	* Must provide styles for core blocks (quote, table, gallery, cover, image captions).
	* Must include block color palette / typography theme supports or `theme.json` configuration.
* **Navigation & Menus:**
...
* **Comments:**
	* Single posts and pages must support comments (`comments_template()`).
...
## 5. Visual Rendering, Edge Cases & Manual QA Checklist
...
## 6. Recommended WP Rig Implementation Plan
...
│                                   WP RIG BUILD SYSTEM                                 │
├───────────────────────────────┬───────────────────────────────┬───────────────────────┤
│    Static PHP Analysis        │   JS / CSS Static Analysis    │  Theme Directory Gate │
├───────────────────────────────┼───────────────────────────────┼───────────────────────┤
...
* `npm run audit:all` — Master task that runs PHPCS, ESLint, Stylelint, i18n checks, and asset/license verification in sequence.
...
## 7. Step-by-Step Theme Review Environment Setup & Playbook
...
### 7.1 Transitioning PHPCS to the WPThemeReview Ruleset
...
### 7.2 Setting Up the Theme Sniffer Plugin
...
### 7.3 Installing the Theme Check Plugin (The Official Review Gate)
...
### 7.4 Runtime Diagnostics & Performance Aids
...
### 7.5 Testing with the Official Theme Unit Test Data
